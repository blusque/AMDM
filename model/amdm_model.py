...
class GaussianDiffusion(nn.Module):
    ...
    def remove_noise(self, xt, pred, ts):
        # print("removed: ", (xt - self.extract(self.remove_noise_coeff, ts, pred.shape) * pred)[0, :10])
        # print("reciprocal_sqrt_alphas: ", self.extract(self.reciprocal_sqrt_alphas, ts, pred.shape))
...
class NoiseDecoder(nn.Module):
    def __init__(
        ...
        for _ in range(layer_num): 
            ...
            linear = nn.Linear(hidden_size + frame_size * 2 + latent_dim, hidden_size)
        ...
        self.fin = nn.Linear(frame_size * 2 + latent_dim, hidden_size)
        self.fco = nn.Linear(hidden_size + frame_size * 2  + latent_dim, frame_size)
